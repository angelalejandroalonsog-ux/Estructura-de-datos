...
template <class T>
pila<T>::pila(){
    tope= 0; //es donde inicia nuestro array, al tener 10 elementos tenemos desde el [[0],[1]...[9]]
}
template <class T>
...
void pila<T>::push(T dato){
    ...
    cout <<"------------------------------------" <<endl;
    cout<<"pisicion en la pila(tope): [" <<tope+1<<"]"<<endl; 
...
void pila<T>::pop(T *dato){//recibimos la dirección de memoria de Valor gracias a que como parametro tenemos declarado un puntero llamado *dato
    ...
    cout <<"dato a eliminar con pop: "<< espacioPila[tope-1]<<endl;
    ...
    *dato= espacioPila[tope--]; /*
                            ...
                            */
    cout <<"Nuevo tope: "<<tope<<endl;
    cout <<"nuevo dato en la cima:  "<< *dato<<endl;
    espacioPila[tope]=0; //borrado del elemento en la posición 9 del arreglo
    cout <<"nuevo dato en la cima (extraido de la pila):  "<< espacioPila[tope-1]<<endl;
    cout <<"tope de la cima (debería estar vacío):  "<< espacioPila[tope]<<endl;
}
...
int main(){
    ...
    if (objPila.pilaVacia()!=1)
    {
        ...
        cout <<"Al tope llegar a 10, va a igualarse con max: "<<max<< endl;
    }
